...
export default function Topbar({
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[60px] items-center gap-2 border-b border-white/5 bg-bg-base/70 px-3 backdrop-blur-glass sm:gap-4 sm:px-7">
      <MobileNav isAdmin={isAdmin} username={username} />

      {/* Brand label on mobile only (in place of hidden sidebar logo) */}
      <div className="flex items-center gap-2 lg:hidden">
        <span className="text-[13px] font-bold tracking-tightest text-ink-primary">
          Tradesure
        </span>
      </div>
